      ...
      <table className='min-w-full divide-y-2 divide-gray-200 bg-white text-sm'>
        ...
        <tbody className='divide-y divide-gray-200'>
          {hours &&
            hours.length > 0 &&
            hours.map((hour: Hour) => {
              const [startHours, startMinutes] = hour.starttime
                .split(':')
                .map(Number)
    ...
    <div className='overflow-x-auto rounded border border-gray-200 mt-4 p-4 shadow'>
      <header>
        ...
        <span className='text-zinc-500'>
          Las horas mostradas son las del centro y mes seleccionados. Por
          defecto se muestran todas las horas en cualquier centro del mes actual
        </span>
        <section className='flex mt-2'>
          <label className='block overflow-hidden rounded-md px-3 py-2 focus-within:border-blue-600 focus-within:ring-1 focus-within:ring-blue-600'>
              ...
              <nav className='-mb-px flex gap-6' aria-label='Tabs'>
                {centers.map((center: WorkCenter) => (
                  <p
                    ...
                    onClick={() =>
                      handleCenterClick(center.workcenterid as number)
                    }
